<template>
  <div class="bg-white px-2 lg:px-6 py-4 relative w-full">
    ...
    <div class="flex items-center justify-between">
      ...
      <button @click="previousMenu" :disabled="startIndex <= 0"
        class="hidden lg:flex items-center justify-center w-10 h-10 rounded-md border border-[#CFCFCF] bg-white hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200">
        <i class="fas fa-arrow-left text-lg text-[#10B981]"></i>
      </button>

      <!-- Menus Container (desktop) -->
      ...
      <!-- Drawer Mobile -->
      <transition name="submenu-slide">
        <div v-if="showMobileMenu" class="fixed top-0 left-0 h-full w-64 bg-white shadow-xl z-50 overflow-y-auto"
    ...
    <transition name="expanded-menus">
      <div v-if="showAllMenus && hiddenMenus.length > 0" class="mt-4 w-full flex flex-wrap justify-center gap-4">
        <transition-group name="expanded-menu-slide" tag="div" class="flex flex-wrap justify-center gap-4" appear>
          <div v-for="menu in hiddenMenus" :key="`expanded-${menu.id}`"
  ...
  <transition name="backdrop">
    <div v-if="activeSubmenu" class="fixed inset-0" @click="closeSubmenu"></div>
  </transition>
</template>
...
const updateResponsive = () => {
  ...
  if (width >= 1536) maxVisibleMenus.value = 4
  else if (width >= 1280) maxVisibleMenus.value = 3
  else if (width >= 1024) maxVisibleMenus.value = 3
  else if (width >= 768) maxVisibleMenus.value = 3
  else maxVisibleMenus.value = 2
}
onMounted(() => {
  updateResponsive()
...
// Méthodes
const previousMenu = () => { if (startIndex.value > 0) { startIndex.value--; activeSubmenu.value = null } }
const nextMenu = () => { if (startIndex.value + maxVisibleMenus.value < allMenus.length) { startIndex.value++; activeSubmenu.value = null } }
const toggleExpandAll = () => { showAllMenus.value = !showAllMenus.value; activeSubmenu.value = null; if (!showAllMenus.value) startIndex.value = 0 }
const toggleSubmenu = (menuId: string) => { activeSubmenu.value = activeSubmenu.value === menuId ? null : menuId }
